...
function makeCsv(rows: Record<string, string>[], extraCols: string[] = []): Buffer {
...
describe('PriceListService', () => {
  ...
  describe('activateRevision', () => {
    it('throws NotFoundException when revision does not exist', async () => {
      mockPrisma.priceListRevision.findUnique.mockResolvedValue(null);
      await expect(service.activateRevision(99)).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ConflictException when revision is already active', async () => {
      ...
      await expect(service.activateRevision(1)).rejects.toThrow(ConflictException);
    });
    ...
    it('archives current active revision and activates the target', async () => {
      ...
      const activated = makeRevision({ status: 'active', activatedAt: new Date() });
      ...
      expect(mockTx.priceListRevision.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { status: 'active' }, data: { status: 'archived' } }),
      );
  ...
  describe('findAll', () => {
    it('uses the active revision when no revisionId given', async () => {
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
    ...
    it('uses the specified revision when revisionId given', async () => {
      mockPrisma.priceListRevision.findUnique.mockResolvedValue(makeRevision({ id: 3 }));
      mockPrisma.priceListItem.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
  ...
  describe('findByCategory', () => {
    it('filters items by category', async () => {
      ...
      expect(mockPrisma.priceListItem.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { category: 'Hearing Aid', void: false } }),
      );
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('returns item with pivoted prices', async () => {
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ id: 1, name: 'NHS Band 1' })]);
  ...
  describe('getPriceForList', () => {
    it('throws NotFoundException when list type does not exist', async () => {
      ...
      await expect(service.getPriceForList('ITEM001', 'Unknown')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws NotFoundException when item does not exist', async () => {
      ...
      await expect(service.getPriceForList('MISSING', 'NHS Band 1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('returns null price when no entry exists for the item+list combination', async () => {
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.priceListType.findUnique.mockResolvedValue(makeListType({ id: 1 }));
  ...
  describe('exportCsv', () => {
    ...
    it('uses specified revision when revisionId given', async () => {
      mockPrisma.priceListRevision.findUnique.mockResolvedValue(makeRevision({ id: 5 }));
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
      mockPrisma.priceListItem.findMany.mockResolvedValue([]);
  ...
  describe('importCsv — validation', () => {
    it('throws BadRequestException for invalid CSV', async () => {
      await expect(
        service.importCsv(Buffer.from('not\x00valid\x00csv\x00\x00\x00'), 'Test', null, null),
      ).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequestException for empty CSV', async () => {
      await expect(
        service.importCsv(Buffer.from('ItemID,Category,Description\n'), 'Test', null, null),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('normalises ItemIDs: strips whitespace, special chars, and uppercases', async () => {
      const csv = makeCsv([{ ItemID: ' item-001 ', Category: 'Cat', Description: 'Desc' }]);
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.itemPrice.findMany.mockResolvedValue([]);
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
    ...
    it('throws BadRequestException when merge=true and no active revision', async () => {
      const csv = makeCsv([{ ItemID: 'ITEM001', Category: 'Cat', Description: 'Desc' }]);
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(null);
      mockPrisma.itemPrice.findMany.mockResolvedValue([]);
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
  ...
  describe('importCsv — warnings', () => {
    ...
    it('does not warn when the band is present in the CSV', async () => {
      ...
      mockPrisma.itemPrice.findMany.mockResolvedValue([]);
      mockPrisma.customer.findMany.mockResolvedValue([{ band: 'NHS Band 1' }]);
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ name: 'NHS Band 1' })]);
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      mockPrisma.priceListItem.upsert.mockResolvedValue({});
      mockPrisma.itemPrice.createMany.mockResolvedValue({});
    ...
    it('does not warn in merge mode when a missing band is covered by the active revision', async () => {
      ...
      mockPrisma.itemPrice.findMany.mockResolvedValue([
        { itemId: 'ITEM001', listId: 1, price: 9.0, list: { name: 'NHS Band 1' } },
      ]);
      mockPrisma.customer.findMany.mockResolvedValue([{ band: 'NHS Band 1' }]);
      mockPrisma.priceListType.findMany.mockResolvedValue([
        makeListType({ id: 2, name: 'Specsavers' }),
      ...
      const result = await service.importCsv(csv, 'Test', null, null, false, true);
      expect(result.warnings).toHaveLength(0);
    });
  });
  ...
  describe('importCsv — new list detection', () => {
    it('identifies lists that are new to the registry', async () => {
      ...
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.aggregate.mockResolvedValue({ _max: { sortOrder: 10 } });
    ...
    it('identifies lists in the registry but absent from the active revision', async () => {
      ...
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ name: 'Specsavers' })]);
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      mockPrisma.priceListItem.upsert.mockResolvedValue({});
      mockPrisma.itemPrice.createMany.mockResolvedValue({});
  ...
  describe('importCsv — dry run', () => {
    ...
    it('still blocks on duplicate ItemIDs in dry run', async () => {
      ...
      await expect(service.importCsv(csv, 'Test', null, null, true)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('returns correct item counts in dry run', async () => {
      ...
      mockPrisma.itemPrice.findMany.mockResolvedValue([
        { itemId: 'ITEM001', listId: 1, price: 5.0, list: { name: 'NHS Band 1' } },
      ]);
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([]);
  ...
  describe('importCsv — replace mode', () => {
    it('creates a draft revision and inserts only CSV prices', async () => {
      const csv = makeCsv(
        [{ ItemID: 'ITEM001', Category: 'Hearing Aid', Description: 'Widget', 'NHS Band 1': '10.50' }],
        ['NHS Band 1'],
      );
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.itemPrice.findMany.mockResolvedValue([]);
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ id: 1, name: 'NHS Band 1' })]);
      const draftRevision = makeRevision({ id: 2, status: 'draft', name: 'My Import' });
      ...
      expect(mockPrisma.priceListRevision.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ status: 'draft', name: 'My Import' }) }),
      );
      expect(mockPrisma.itemPrice.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([
            expect.objectContaining({ itemId: 'ITEM001', listId: 1, price: 10.5, revisionId: 2 }),
          ]),
        }),
      );
    ...
    it('registers new list types before inserting prices', async () => {
      const csv = makeCsv(
        [{ ItemID: 'ITEM001', Category: 'Cat', Description: 'Desc', 'New Band': '5.00' }],
        ['New Band'],
      );
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.itemPrice.findMany.mockResolvedValue([]);
      ...
      mockPrisma.priceListType.aggregate.mockResolvedValue({ _max: { sortOrder: 5 } });
      mockPrisma.priceListType.create.mockResolvedValue({});
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      mockPrisma.priceListItem.upsert.mockResolvedValue({});
      mockPrisma.itemPrice.createMany.mockResolvedValue({});
      ...
      expect(mockPrisma.priceListType.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ name: 'New Band', sortOrder: 6 }) }),
      );
    });
    ...
    it('passes importedBy as createdBy when registering new list types', async () => {
      ...
      mockPrisma.priceListType.findMany
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([makeListType({ id: 99, name: 'New Band' })]);
      mockPrisma.priceListType.aggregate.mockResolvedValue({ _max: { sortOrder: 0 } });
      mockPrisma.priceListType.create.mockResolvedValue({});
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      mockPrisma.priceListItem.upsert.mockResolvedValue({});
      mockPrisma.itemPrice.createMany.mockResolvedValue({});
      ...
      expect(mockPrisma.priceListType.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ name: 'New Band', createdBy: 'jsmith' }),
        }),
      );
    });
  });
  ...
  describe('importCsv — merge mode', () => {
    it('seeds priceMap with active revision prices then overlays CSV', async () => {
      const csv = makeCsv(
        [{ ItemID: 'ITEM001', Category: 'Cat', Description: 'Desc', 'NHS Band 1': '15.00' }],
        ['NHS Band 1'],
      );
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      // Active revision has ITEM001/Band1 at 10 and ITEM002/Band1 at 20
      mockPrisma.itemPrice.findMany.mockResolvedValue([
        { itemId: 'ITEM001', listId: 1, price: 10.0, list: { name: 'NHS Band 1' } },
        { itemId: 'ITEM002', listId: 1, price: 20.0, list: { name: 'NHS Band 1' } },
      ]);
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ id: 1, name: 'NHS Band 1' })]);
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      ...
      const item1Price = prices.find((p: any) => p.itemId === 'ITEM001' && p.listId === 1);
      expect(item1Price?.price).toBe(15.0);

      // Carried forward: ITEM002/Band1 should be present at 20
      const item2Price = prices.find((p: any) => p.itemId === 'ITEM002' && p.listId === 1);
    ...
    it('returns listsCarriedForward for lists not in the CSV', async () => {
      const csv = makeCsv(
        [{ ItemID: 'ITEM001', Category: 'Cat', Description: 'Desc', Specsavers: '5.00' }],
        ['Specsavers'],
      );
      mockPrisma.priceListRevision.findFirst.mockResolvedValue(makeRevision({ id: 1 }));
      mockPrisma.itemPrice.findMany.mockResolvedValue([
      ...
      mockPrisma.customer.findMany.mockResolvedValue([]);
      mockPrisma.priceListType.findMany.mockResolvedValue([makeListType({ id: 2, name: 'Specsavers' })]);
      mockPrisma.priceListRevision.create.mockResolvedValue(makeRevision({ id: 2, status: 'draft' }));
      mockPrisma.priceListItem.upsert.mockResolvedValue({});
      mockPrisma.itemPrice.createMany.mockResolvedValue({});
  ...
  describe('voidItem', () => {
    it('throws NotFoundException when item does not exist', async () => {
      mockPrisma.priceListItem.findUnique.mockResolvedValue(null);
      await expect(service.voidItem('MISSING')).rejects.toThrow(NotFoundException);
    });

    it('throws BadRequestException when item is already voided', async () => {
      mockPrisma.priceListItem.findUnique.mockResolvedValue(makeItem({ void: true }));
      await expect(service.voidItem('ITEM001')).rejects.toThrow(BadRequestException);
    });

    it('sets void=true, voidDateStamp, and voidedBy', async () => {
      mockPrisma.priceListItem.findUnique.mockResolvedValue(makeItem());
      const voided = makeItem({ void: true, voidDateStamp: new Date(), voidedBy: 'jsmith' });
  ...
  describe('voidListType', () => {
    ...
    it('throws BadRequestException when list type is already voided', async () => {
      mockPrisma.priceListType.findUnique.mockResolvedValue(makeListType({ void: true }));
      await expect(service.voidListType(1)).rejects.toThrow(BadRequestException);
    });

    it('sets void=true, voidDateStamp, and voidedBy', async () => {
      mockPrisma.priceListType.findUnique.mockResolvedValue(makeListType());
      const voided = makeListType({ void: true, voidDateStamp: new Date(), voidedBy: 'jsmith' });
  ...
  describe('void filtering', () => {
    ...
    it('findOne throws NotFoundException for a voided item', async () => {
      ...
      await expect(service.findOne('ITEM001')).rejects.toThrow(NotFoundException);
    });
    ...
    it('getPriceForList treats a voided list type as not found', async () => {
      ...
      await expect(service.getPriceForList('ITEM001', 'NHS Band 1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
