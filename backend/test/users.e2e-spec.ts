...
describe("Users (e2e)", () => {
  ...
  describe("self-service", () => {
    ...
    it.each([Role.Admin, Role.Manager, Role.Operative, Role.ReadOnly, Role.Customer])(
      "%s can read own profile (passwordHash excluded)",
      async (role) => {
  ...
  describe("validation (POST /users)", () => {
    ...
    it("rejects a password under MinLength(8)", () =>
      create({ username: `${E2E_MARKER}short`, password: "short" }).expect(400));
    ...
    it("rejects an unknown property", () =>
      create({ username: `${E2E_MARKER}x`, password: "Password1!", bogus: 1 }).expect(
        400,
      ));
  });
  ...
  describe("user management lifecycle", () => {
    ...
    it("unlocks the account", () =>
      api(app).patch(`/api/users/${userId}/unlock`).set(as(Role.Admin)).expect(200));
